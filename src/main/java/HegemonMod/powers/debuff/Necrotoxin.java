...
import HegemonMod.actions.NecrotoxinLoseHPAction;
...
public class Necrotoxin extends BasePower implements HealthBarRenderPower {
    public static final String POWER_ID = ("HegemonMod:" + Necrotoxin.class.getSimpleName());
    ...
    @Override public void updateDescription() {
        ...
        if (owner.hasPower(Sin.POWER_ID))
            this.description += "#g" + calculateSinInt(owner, amount) + DESCRIPTIONS[1] + calculateSinInt(owner, amount * (amount + 1) / 2);
        else
            this.description += "#b" + amount + DESCRIPTIONS[1] + amount * (amount + 1) / 2;
        this.description += DESCRIPTIONS[2];
    }

    @Override public int getHealthBarAmount() { return calculateSinInt(owner, this.amount); }

    @Override public Color getColor() { return CardHelper.getColor(50, 25, 75); }
